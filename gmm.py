...
class GMM(object):
    ...
    def _ll_joint(self, points, pi, mu, sigma, **kwargs):  # [10pts]
        ...
        for j in range(K):
            normal_log = np.log(self.multivariate_normal(points, mu[j], sigma[j])+1e-64, dtype=np.float64)
    ...
    def multivariate_normal(self, x, mu, cov):
        ...
        return np.exp(-B / 2) / np.sqrt(((2 * np.pi) ** x.shape[1]) * (np.linalg.det(cov)))
    ...
    def _M_step(self, points, gamma, **kwargs):  # [10pts]
        ...
        for k in range(gamma.shape[1]):
            ...
            sigma = np.dot(A, x_scaled) / gamma[:, k].sum(0)
        return pi, mu, sigma
