...
class NaiveBayes(object):
    ...
    def _likelihood_ratio(self, XH, XM): # [5pts]
        ...
        '''
        XH_sum = (XH.sum(axis=0) + 1.)/(XH.sum()+XH.shape[1])
        XM_sum = (XM.sum(axis=0)+1.)/(XM.sum()+XM.shape[1])
        return XH_sum/XM_sum
    ...
    def classify_disputed(self, fratio, pratio, XD): # [5pts]
        ...
        '''
        fratio_xd = np.prod(np.power(fratio,XD).astype(np.float64),axis=1)*pratio
        results = []
        for el in fratio_xd:
            if el > 1:
                results.append('H')
